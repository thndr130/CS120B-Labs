...
enum IncDec_States {ID_start, wait, inc, dec} ID_state;
...
unsigned short count_ID = 0x00;
unsigned char temp = 0x00;
unsigned short freq = 0x02;
...
void Speaker(){
	switch(speaker_state){
		case S_start:
			...
			break;
		case S_on:
			if(((~PINA & 0x04) == 0x04) && (count_speaker < freq)) {
				speaker_state = S_on;
				++count_speaker;		
			}
...
void IncDec(){
	switch(ID_state){
		case ID_start:
			...
			break;
		case wait:
			if (((~PINA & 0x01) == 0x01) && freq < 7 && count_ID > 1000){
				ID_state = inc;
				count_ID = 0;
			}
			else if(((~PINA & 0x02) == 0x02) && freq > 0 && count_ID > 1000){
				ID_state = dec;
				count_ID = 0;
			}
			else {
